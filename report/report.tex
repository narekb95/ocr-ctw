...
Our solver removes isolated vertices from the graph at the beginning, and builds a new graph $G'$. This ensures that each vertex has at least one neighbor. The solver sorts the adjacency lists of each vertex by the order they appear on the linear arrangement and assigns a range to each vertex, given by the first and the last index (in the linear arrangement) of a neighbor of this vertex. This allows to compute crossing numbers between two vertices in polynomial time in $k$.
